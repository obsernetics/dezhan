...
--  state machine never reads a system clock. Legal hold is marked "future" in
--  the spec and is intentionally NOT implemented here.
...
package Dezhan.Trusted_Core.Retention with SPARK_Mode is
   ...
   --  The deletion policy. Its definition IS the enforcement rule.
   function Can_Delete
     (L    : Retention_Lock;
      Now  : Trusted_Time;
   ...
   is
     (case L.Mode is
         ...
         when Governance => Now >= L.Retain_Until or else Auth.Bypass_Governance);
