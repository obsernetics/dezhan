#!/usr/bin/env python3
# Render one grouped bar chart (log scale) of PUT/GET throughput for dezhan and
# MinIO into an SVG. No external dependencies. Single-node measurement: both
# servers on the same VM, same harness (see results/COMPARISON.md).
import json, math

ENVS = ["dezhan-vm", "minio-vm"]
LABELS = {"dezhan-vm": "dezhan", "minio-vm": "MinIO"}
COLORS = {"dezhan-vm": "#2a7ae2", "minio-vm": "#e67e22"}
...
s.append(f'<text x="{L}" y="24" font-size="16" font-weight="bold">dezhan vs MinIO: S3 throughput (objects/s, log scale)</text>')
...
for i, e in enumerate(ENVS):
    ...
    s.append(f'<text x="{L+pw+40}" y="{ly+12}">{LABELS[e]}</text>')
s.append(f'<text x="{L+pw+20}" y="{T+ph+18:.0f}" font-size="11" fill="#666">single VM, 4 vCPU</text>')
s.append('</svg>')
